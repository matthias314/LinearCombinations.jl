...
export DenseLinear, basis, coordinates
...
"""
    basis(a::DenseLinear) -> Basis

Return the basis specified when `a` was defined.

See also [`coordinates`](@ref).
"""
...
Return the coordinates of `a` with respect to the basis used for it.
The coordinate array (usually a vector) has the same axes as the basis.
Modifying this array will also modify `a`.
...
@inline function iterate(a::DenseLinear{T,R}, state...) where {T,R}
